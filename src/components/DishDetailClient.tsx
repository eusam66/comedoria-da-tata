...
export default function DishDetailClient({ dish }: { dish: Dish }) {
  ...
  const unavailable = dish.isAvailable === false || Number(dish.stock ?? 1) < 1;
    ...
    <article className="mx-auto grid max-w-5xl overflow-hidden rounded-[2rem] border border-brand-brown/10 bg-white shadow-[0_24px_70px_-38px_rgba(42,20,15,0.5)] lg:grid-cols-2">
      <div className="relative">
        {dish.image ? (
          ...
          <div className="flex aspect-[4/3] items-center justify-center bg-brand-beige text-sm font-semibold text-brand-brown">
            Imagem indisponível
          </div>
        )}
      ...
      <div className="p-5 sm:p-7 lg:p-10">
        ...
        <h1 className="mt-2 font-display text-3xl leading-tight text-brand-dark sm:text-4xl">
          {dish.name}
        </h1>
        {dish.description && (
          <p className="mt-4 leading-7 text-brand-brown/80">{dish.description}</p>
        )}

        <div className="mt-6 flex flex-wrap items-end justify-between gap-4 border-y border-brand-brown/10 py-5">
          <div>
            <p className="text-xs font-semibold uppercase tracking-wider text-brand-brown/60">
              Preço
            </p>
          ...
          {dish.servings != null && (
            <div className="text-right">
              ...
              <p className="mt-1 font-semibold text-brand-dark">
                {dish.servings} {dish.servings === 1 ? 'pessoa' : 'pessoas'}
              </p>
            </div>
          )}
        </div>

        {typeof dish.ingredients === 'string' && dish.ingredients.trim().length > 0 && (
          <section className="mt-6" aria-labelledby="acompanhamentos-title">
            <h2
              id="acompanhamentos-title"
              className="text-sm font-bold uppercase tracking-wider text-brand-brown/70"
            >
              Acompanhamentos
            </h2>
            <p className="mt-2 leading-7 text-brand-dark">{dish.ingredients}</p>
          </section>
        )}
            ...
            <div className="mt-3 space-y-3">
              {addons.map((addon) => {
                ...
                return (
                  <div
                    key={addon.id}
                    className="flex items-center justify-between gap-3 rounded-2xl border border-brand-brown/10 bg-brand-beige/45 p-3"
                  >
                    <div className="min-w-0">
                      <p className="font-semibold text-brand-dark">{addon.name}</p>
                      <p className="mt-0.5 text-sm text-brand-brown/65">
                        {addon.price > 0 ? `+ R$ ${addon.price.toFixed(2)}` : 'Incluso'}
                        {addon.required ? ' · obrigatório' : ''}
                      </p>
                    </div>
                    <div className="flex shrink-0 items-center overflow-hidden rounded-full border border-brand-brown/15 bg-white">
                      <button
                        ...
                        onClick={() => changeAddonQty(addon, addonQty - 1)}
                        disabled={addonQty <= (addon.required ? 1 : 0)}
                        className="h-10 w-10 text-lg text-brand-dark disabled:opacity-35"
                      >
                        −
                      </button>
                      <span className="min-w-8 text-center text-sm font-bold text-brand-dark">
                        {addonQty}
                      </span>
                      <button
                        type="button"
        ...
        <div className="mt-8">
          ...
          <div className="mt-2 inline-flex items-center overflow-hidden rounded-full border border-brand-brown/15 bg-brand-beige/50">
            <button
              ...
              disabled={qty === 1}
              className="h-12 w-12 bg-white text-xl text-brand-dark disabled:opacity-35"
            >
              −
            </button>
            ...
            <button
              ...
              onClick={() => setQty((current) => Math.min(Number(dish.stock ?? 20), current + 1))}
              disabled={qty >= Number(dish.stock ?? 20)}
              className="h-12 w-12 bg-white text-xl text-brand-dark disabled:opacity-35"
            >
              +
            </button>
        ...
        <div className="mt-7 grid gap-3 sm:grid-cols-2">
          <button
            ...
            className="min-h-14 rounded-full bg-brand-orange px-6 py-3 font-bold text-white shadow-lg shadow-brand-orange/20 transition hover:bg-brand-dark disabled:cursor-not-allowed disabled:opacity-50"
          >
            {unavailable ? 'Esgotado' : `Adicionar à sacola · R$ ${total.toFixed(2)}`}
          </button>
          <button
            type="button"
            onClick={handleShare}
            disabled={sharing}
            className="min-h-14 rounded-full border border-brand-brown/20 bg-white px-6 py-3 font-bold text-brand-brown transition hover:bg-brand-beige disabled:opacity-60"
          >
            {sharing ? 'Compartilhando…' : 'Compartilhar prato'}
          </button>
        </div>
      </div>
    </article>
