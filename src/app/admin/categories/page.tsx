...
export default function AdminCategoriesPage() {
  ...
  useEffect(()=>{ fetch('/api/admin/categories').then(r=>r.json()).then(setItems).catch(e=>setError(e.message)); },[]);
  ...
  async function create(){
    ...
    try{
      ...
      setItems((s)=>[json,...s]); setName('');
      toastSuccess('Categoria criada');
    } catch(e:any){ setError(e.message); toastError(e?.message || 'Erro'); }
  }
  async function remove(id:string){
    ...
    } catch(e:any){ setError(e.message); toastError(e.message); }
  }
  ...
  return (
    <AdminLayout>
      ...
      <div className="bg-white p-4 rounded shadow mb-6">
        ...
        {step === 2 && (
          <div>
            <div className="mb-3">Confirme a criação da categoria: <strong>{name}</strong></div>
            <div className="flex gap-2">
              <button type="button" onClick={()=>setStep(1)} className="flex-1 border py-3 rounded">Voltar</button>
              <button type="button" onClick={async ()=>{ await create(); setStep(1); }} className="flex-1 bg-brand-dark text-white py-3 rounded">Criar</button>
            </div>
          </div>
        )}
